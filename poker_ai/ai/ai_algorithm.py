...
def rule_based_with_simulation_ai(player, num_players, board, actions, cur_call, cur_raise, mul_indicator):
    """Return the actions that the AI do base on the list of actions that it can do.
    ...
    for _ in range(DECIDER):
        decides.append(random.random())
    decide = sum(decides)/DECIDER
    if pot_odd > draw: # Need to call more money than hand worth when draw or win -> fold if can't check
        if 2 in actions:
            return [2]
        return [5]
    else: # Hand worth when not lose is more than % of money need to call -> play
        if pot_odd <= draw and pot_odd > win:
            if 2 in actions:
                return [2]
            elif 3 in actions:
                if cur_call-player.pot <= CONFIDENT_RANGE*player.money:
